...
def test_mellophone_greeting(host):
    cmd = "curl -o -I -L -s -w "%{http_code}\n" 'http://localhost:8080/mellophone'"
...
def test_mellophone_works(host):
    cmd = 'curl -o -I -L -s -w "%{http_code}\n"' +\
          "'localhost:8080/mellophone/checkcredentials?pwd=kulkov&login=kulkov'"
    assert host.check_output(cmd) == '200'
    cmd = 'curl -o -I -L -s -w "%{http_code}\n"' +\
          "'localhost:8080/mellophone/checkcredentials?pwd=kulkov&login=kulkov2'"
